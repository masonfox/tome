...
A user wants to set a goal for how many books they aim to read during a specific calendar year. They navigate to the Settings page, find the Reading Goals section, and enter their target number of books along with the year they want to track.
...
A user with an active reading goal wants to see their current progress at a glance. When they visit the dashboard (home page), they see a Reading Goal widget displaying their target, current progress (X / Y books), a visual progress bar, and an indicator of whether they're on track, ahead, or behind schedule.
...
5. **Given** a user is viewing their progress mid-year, **When** they hover over or tap the pace indicator, **Then** they see a projected finish date message like "At this pace, you'll finish by November 15, 2026"
...
- What happens on January 1st when switching to a new year? If the user has a goal for the new year, the dashboard displays that goal with 0 books completed. Previous year's goal remains accessible in Settings for historical reference.
...
- **FR-010**: System MUST calculate projected finish date based on current reading pace (books per day)
...
- **Completed Book**: Derived from existing reading sessions/book completion tracking. Key attribute: completion date (used to determine which year a book counts toward). Relationship: Multiple completed books contribute to one annual goal based on matching year.
...
- Users track book completion through an existing "mark as read" or "reading session" feature that records completion dates
