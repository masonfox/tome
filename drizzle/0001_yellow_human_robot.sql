...
CREATE TABLE `__new_reading_sessions` (
	...
	CONSTRAINT "rating_check" CHECK("__new_reading_sessions"."rating" IS NULL OR ("__new_reading_sessions"."rating" >= 1 AND "__new_reading_sessions"."rating" <= 5))
);
--> statement-breakpoint
INSERT INTO `__new_reading_sessions`("id", "user_id", "book_id", "session_number", "status", "started_date", "completed_date", "rating", "review", "is_active", "created_at", "updated_at") SELECT "id", "user_id", "book_id", "session_number", "status", "started_date", "completed_date", "rating", "review", "is_active", "created_at", "updated_at" FROM `reading_sessions`;--> statement-breakpoint
