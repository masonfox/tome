...
describe("ShelfRepository - Status Display", () => {
  ...
  describe("moveBookToBottom", () => {
    it("should move book to bottom position and shift others up", async () => {
      // Create shelf
      const shelf = await shelfRepository.create({
        name: "Move to Bottom Test Shelf",
        userId: null,
      });
      ...
      await shelfRepository.moveBookToBottom(shelf.id, books[0].id);
    ...
    it("should handle moving already bottom item (no-op)", async () => {
      const shelf = await shelfRepository.create({
        name: "Already Bottom Test Shelf",
        userId: null,
      });
      ...
      // Move last book (already at bottom) to bottom
      await shelfRepository.moveBookToBottom(shelf.id, books[1].id);
    ...
    it("should work with single book on shelf", async () => {
      ...
      await shelfRepository.moveBookToBottom(shelf.id, book!.id);
    ...
    it("should handle moving from middle position", async () => {
      ...
      await shelfRepository.moveBookToBottom(shelf.id, books[2].id);
    ...
    it("should throw error if book not on shelf", async () => {
      ...
      // Try to move book that's not on shelf
      await expect(
        shelfRepository.moveBookToBottom(shelf.id, book!.id)
      ).rejects.toThrow(/Book .+ is not on shelf .+/);
    });
    ...
    it("should only affect books on the specified shelf", async () => {
      ...
      await shelfRepository.moveBookToBottom(shelf1.id, books[0].id);
